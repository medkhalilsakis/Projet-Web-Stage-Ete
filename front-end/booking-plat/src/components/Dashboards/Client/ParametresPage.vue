  ...
  export default {
    ...
    methods: {
      ...
      async deleteUser() {
        ...
        if (result.isConfirmed) {
          try {
            const reservationsResponse = await axios.get(`http://127.0.0.1:8000/api/reservations/client/${this.user.IDCARD}`);
            const reservations = reservationsResponse.data;
  
            if (reservations.length > 0) {
              await axios.delete(`http://127.0.0.1:8000/api/reservations/client/${this.user.IDCARD}`);
            ...
            await axios.delete(`http://127.0.0.1:8000/api/users/${this.user.id}`);
